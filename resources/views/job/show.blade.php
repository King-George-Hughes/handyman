<x-layout.layout>
     ...
     <div class="relative h-[300px] md:h-[500px]">
        ...
        <!-- Search container -->
        <div
          class="absolute w-full h-full flex justify-center items-center md:h-[500px] sm:h-[300px]"
        >
      ...
      <div class="w-full grid grid-cols-6 px-6 gap-5">
        <div class="w-full col-span-6 md:col-span-4">
          ...
          <div class="pt-4">
            ...
            <span class="text-sm p-1 font-bold">
              <i class="fa-solid fa-location-dot fa-1x mt-5"></i> {{$job->region->name}}, {{$job->location->name}}
            </span>
          </div>
        </div>
        ...
        <div class="w-full col-span-6 md:col-span-2 flex-col md:justify-center">
          <div class="md:mt-0 md:p-5 md:border-[1px] h-fit rounded-md">
            <div
              class="sm:space-x-4 flex md:flex-col md:space-y-[10px] items-center"
            >
              <button
                class="bg-brightRedLight rounded-full px-20 py-2 text-white md:px-12"
              >
                <span
              ...
              <a
              href="tel:{{$job->user->contact}}"
                class="bg-brightRed rounded-full px-14 py-2 text-white md:px-12"
              >
                <span class=""
                  ><i class="fa-solid fa-phone"></i> {{$job->user->contact}}</span
                >
              </a>
            </div>
          </div>
          <div
            class="mt-7 p-5 bg-darkGrayishBlue col-span-6 md:col-span-2 rounded-md"
          >
            <p>
              {{$job->description}}
            </p>
